...
// netlify/functions/coach-announcements.mjs
var coach_announcements_exports = {};
__export(coach_announcements_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(coach_announcements_exports);
...
var handler = async (event) => {
  ...
  if (method === "POST") {
    ...
    try {
      ...
      const announcement = await createCoachAnnouncement({
        coachId: coach.id,
        title,
        body: text
      });
      ...
      if (detail.includes("NOT_FOUND") || detail.includes("UNKNOWN_FIELD")) {
        return wrapCoachResponse(
          gate,
          json(503, {
            error: "Announcements table is not set up yet. Add a Coach Announcements table in Airtable (see README)."
          })
        );
      }
...
//# sourceMappingURL=coach-announcements.js.map
