...
// netlify/functions/qr-checkins.mjs
var qr_checkins_exports = {};
__export(qr_checkins_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(qr_checkins_exports);
...
function sign2(payload) {
  return import_node_crypto2.default.createHmac("sha256", sessionSecret2()).update(payload).digest();
}
function createCoachSessionCookieValue(email) {
  ...
  const signature = base64UrlEncode(sign2(payload));
  return `${payload}.${signature}`;
}
function safeEquals2(a, b) {
  if (a.length !== b.length) return false;
...
function readCoachSessionCookieValue(cookieValue) {
  ...
  if (parts.length !== 2) return null;
  const [payload, signature] = parts;
  const expectedSig = sign2(payload);
  let providedSig;
  try {
    providedSig = base64UrlDecode2(signature);
  } catch {
    return null;
  }
  if (!safeEquals2(expectedSig, providedSig)) return null;
  let parsed;
  try {
    parsed = JSON.parse(base64UrlDecode2(payload).toString("utf8"));
  } catch {
    return null;
  }
...
// netlify/functions/qr-checkins.mjs
var CHECKIN_METHOD_BY_INPUT = {
  "Fallback Code": "Fallback Code",
  "QR Code": "QR Code"
};
var VALID_SCAN_TYPES = /* @__PURE__ */ new Set(["Arrival", "Departure"]);
var KIT_REMINDER_DELAY_MIN = 5;
var KIND_KIT_REMINDER = "Kit Reminder";
var handler = async (event) => {
  ...
  try {
    ...
    if (scanType === "Departure" && parentId) {
      try {
        const sendAt = new Date(Date.now() + KIT_REMINDER_DELAY_MIN * 6e4);
    ...
    return authenticatedCoachGate ? wrapCoachResponse(authenticatedCoachGate, okResponse) : okResponse;
  } catch (error) {
...
//# sourceMappingURL=qr-checkins.js.map
