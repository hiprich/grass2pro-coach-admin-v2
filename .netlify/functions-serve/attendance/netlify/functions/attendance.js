...
// netlify/functions/attendance.mjs
...
var handler = async (event) => {
  ...
  const sessionId = event.queryStringParameters?.sessionId || void 0;
  try {
    const attendance = await listAttendance({ sessionId });
    return wrapCoachResponse(
      gate,
      json(200, { attendance, sessionId: sessionId || null, updatedAt: (/* @__PURE__ */ new Date()).toISOString() })
    );
  } catch (error) {
